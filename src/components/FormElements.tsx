...
export function Input({ name, label, className, required, ...props }: InputProps) {
  ...
  return (
    <div className={cn("space-y-1.5", className)}>
      ...
      <input
        ...
        {...props}
      />
      {error && <p className="text-xs text-rose-500 font-semibold ml-1 mt-1">{error.message}</p>}
    </div>
  );
}
...
export function Select({ name, label, options, className, required, ...props }: SelectProps) {
  const { register, formState: { errors } } = useFormContext();
  const error = (errors as any)[name.split('.')[0]]?.[name.split('.')[1]] || (errors as any)[name];
    ...
    <div className={cn("space-y-1.5", className)}>
      ...
      <div className="relative">
        <select
          {...register(name)}
          className={cn(
            "w-full bg-slate-50 dark:bg-slate-800/50 border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-logistics-blue transition-all outline-none appearance-none pr-10",
            error && "ring-2 ring-rose-500"
          )}
          {...props}
        >
